...
/**
 * Die Klasse Server implementiert ein eigenstaendiges Programm, das auf einem Rechner gestartet wird, dessen Adresse
 * den Spielern bekannt ist. Es soll bis zu 50 Verbindungen mit Clients annehmen und die auf diesen laufenden
...
public class Server {
    ...
    /**
     * Konstruktor, initialisiert die Variable servSocket durch Erzeugen eines ServerSocket-Objekts auf dem angegebenen
     * Port. Desweiteren wird der Vektor "games" initialisiert, der die Spiele vom Typ Game enthalten wird.
     *
    ...
    /**
     * In dieser Methode wird eine neue Instanz vom Typ Server erzeugt. Danach werden innerhalb einer Endlosschleife
     * Verbindungen angenommen, denen von der Methode addPlayer ein eigener Thread zugewiesen wird und die in der Liste
     * "games" verwaltet werden.
     *
    ...
    /**
     * Diese Methode dient zum Aufnehmen eines neuen Spielers. Sie verarbeitet die in "data" enthaltenen Spielerdaten
     * (Name des Spiels, minimale und maximale Spieleranzahl des Spiels)  und entscheidet dann, ob der Spieler in ein
     * bestehendes Spiel vom Typ Game hinzugefuegt werden kann, oder ob ein neues Objekt vom Typ "Game" erzeugt werden
     * muss. Dann wird ein eigener Thread fuer den neuen Spieler erzeugt, der in der "players" Liste des jew.
    ...
    /**
     * Diese Methode loescht einen Spieler aus einem Spiel, wenn dieser z.B. seinen Client vor Beginn des Spiels beendet
     * hat. Dazu wird die delete-Methode des entsprechenden Game-Objekts aufgerufen.
     *
    ...
    /**
     * Diese Methode dient zum Weiterleiten von Nachrichten zum Zielspieler, der im String "data" enthalten ist. Sie
     * wird vom sendenden ServerThread aufgerufen und schickt die Nachricht ueber den Ziel-ServerThread an das
     * entsprechende Communicator-Objekt des Empfaengers. Die Variable "gameID" (s. Klasse ServerThread) dient der
     * Methode zur Unterscheidung von mehreren Spielen des gleichen Typs. Au�erdem werden Meldungen, die an den Server
     * direkt gehen, an die entsprechenden Methoden des Servers weitergeleitet ("Spiel starten" und "Client beendet").
     * Diese Meldungen erkennt die Methode daran, dass die Zieladresse im String "data" (normalerweise die Spielernummer
     * des Empfaengers) negativ ist.
     *
    ...
    /**
     * Diese Methode unterbricht die Verbindung zu den Spielern eines Spiels. Sie wird vom ServerThread des Spielers
     * aufgerufen, dessen Client die Verbindung als erster unterbrochen hat. Nach Aufruf der Methode shutdown() in
     * Klasse Game wird dann das Spiel ggf. aus der Liste geloescht.
     *
