 ...
 * <p>
 * Its methods allow the user to obtain a new certificate based on his system login information. The issued certificates
 * are objects of the <cite>Certificate</cite>-class. <br> There are also methods to access the public part of the
 * trusted authority's RSA key.
 *
...
public class TrustedAuthority {
    ...
    /**
     * Issues a new certificate to the caller containing the given data. The issued certificate contains the username
     * under which the program calling this method is running. If no such username can be obtained a random String of
     * length 50 is used as ID. The signature of the certificate is obtained by applying first the SHA hash function to
     * the ID-String followed by data, see the <cite>MessageDigest</cite>-class. That is, if <code>md</code> is the SHA
     * <cite>MessageDigest</cite>-object and <code>ID</code> is the ID-String, then <code>newCertificate</code> will
     * call <br> <br> <code>md.update(ID.getBytes());</code><br> <code>md.update(data);</code><br> <code>digest =
     * md.digest();</code><br><br> After that the resulting message digest <code>digest</code> of 128 bits is converted
     * to a <cite>BigInteger</cite>-object by using the appropriate constructor and signed with the secret RSA key of
     * the trusted authority. <strong>Important:</strong> When checking a certificate notice that because of the usage
     * of RSA the two values to be compared are only equal modulo the trusted authorities modulus
     * <cite>getModulus</cite>.
     *
