...
/**
 * This class provides some special methods for BigInteger arithmetic that are useful for certain cryptographic
 * algorithms.
 *
...
public class ProtocolUtils extends BigIntegerUtil {
    ...
    /**
     * Hashes an identification string I and an integer j to a BigInteger of [0, max - 1], i.e. [0, 2^(max.bitLength ()
     * - 1) - 1] where max / 2 < 2^(max.bitLength () - 1) = 2^( [lb(max)] ) <= max
     *
    ...
    /**
     * This routine yields a solution to the Chinese Remainder Theorem (CRT), as described in Algorithmus 3.4. This
     * special case describes the application with t=2.
     *
     ...
     * <code>modulus</code> must be a prime number, and <code>val</code> a quadratic remainder. Then, an x out of [1,
     * ..., modulus-1] is calculated with x**2 mod <code>modulus</code> = a mod <code>modulus</code>.
     *
    ...
    public static BigInteger sqrtMod(BigInteger val, BigInteger modulus) {
        ...
        do {
            b = BigIntegerUtil.randomBetween(ONE, modulus);
    ...
    private static BigInteger[] sucheLM(BigInteger p) {
        ...
        lm[0] = ZERO;
        lm[1] = p.subtract(ONE);
        while (Objects.equals(lm[1].gcd(BigInteger.valueOf(2L)), BigInteger.valueOf(2L))) {
    ...
    private static BigInteger sucheXkleinerN(BigInteger n) {
        ...
        do {
            x = BigIntegerUtil.randomBetween(ONE, n.subtract(ONE));
